...
class VesselEnv(gym.Env):
    ...
    def __init__(self, vessel: Vessel, initial_state: np.ndarray, dT: float, target_position: np.ndarray, render_mode='human', max_steps=200, slow_down_distance=10.0):
        ...
        self.target_psi = np.deg2rad(-90)

        self.psi = np.deg2rad(-90)  # Assume initial heading is zero
        self.position = np.array([50.0, 0.0])  # Assume initial position is at the origin
